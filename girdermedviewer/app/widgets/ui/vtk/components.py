...
class SliceView(VtkView):
    ...
    def add_primary_volume(self, image_data, data_id=None):
        reslice_image_viewer = render_volume_in_slice(
            image_data, self.renderer, self.orientation.value, obliques=self.state.obliques_visibility
        )
    ...
    def set_volume_scalar_color_preset(self, data_id: str, preset_name: str, is_inverted: bool):
        ...
        if reslice_image_viewer is not None or preset is None:
            modified = self.color_preset_parser.apply_preset_to_volume(preset, reslice_image_viewer, is_inverted)
        for slice in self.get_image_slices(data_id):
            modified = self.color_preset_parser.apply_preset_to_volume(preset, slice.GetProperty(), is_inverted)
        if modified:
            self.update()
...
class QuadView(v3.VContainer):
    ...
    def _build_ui(self):
        with (
            ...
            with SliceView(
                Orientation.SAGITTAL, ref="sag_view", v_if="fullscreen == null || fullscreen == 'sag_view'"
            ) as sag_view:
                self.views.append(sag_view)
            with ThreeDView(ref="threed_view", v_if="fullscreen == null || fullscreen == 'threed_view'") as threed_view:
                self.views.append(threed_view)
            with SliceView(
                Orientation.CORONAL, ref="cor_view", v_if="fullscreen == null || fullscreen == 'cor_view'"
            ) as cor_view:
                self.views.append(cor_view)
            with SliceView(
                Orientation.AXIAL, ref="ax_view", v_if="fullscreen == null || fullscreen == 'ax_view'"
            ) as ax_view:
