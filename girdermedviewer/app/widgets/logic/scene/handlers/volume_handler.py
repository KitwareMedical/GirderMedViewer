...
class VolumeDisplayHandler:
    ...
    def update_visibility(self, volume_id: str) -> Callable:
        @debounce(0.05)
        def _update_visibility(visible: bool) -> None:
            for view in self.views:
                view.set_volume_visibility(volume_id, visible)
...
class VolumeHandler(ObjectHandler):
    def __init__(self, server):
        ...
        self.object_logics: dict[str, VolumeObjectLogic] = {}

        self.bind_changes({(self.name.active_primary_volume_id,): self._update_primary_volumes_visibility})
    ...
    def _update_primary_volumes_visibility(self, active_primary_volume_id: str | None) -> None:
        for volume_id in self.primary_volume_ids:
            volume_logic = self.object_logics.get(volume_id)
            volume_logic.scene_object.is_visible = volume_id == active_primary_volume_id
    ...
    def _connect_volume_to_display_handler(self, volume_logic: VolumeObjectLogic):
        ...
        volume_logic.scene_object.watch(("is_visible",), self.display_handler.update_visibility(volume_logic._id))
        volume_logic.updated.connect(self.display_handler.update_views)
    ...
    def _get_next_volume(self, volume_logic: VolumeObjectLogic) -> tuple[str, bool]:
        ...
        for vol_id, vol in self.object_logics.items():
            if vol.soft_parent_id == volume_logic._id:
    ...
    def add_object_to_views(self, volume_logic: VolumeObjectLogic) -> None:
        ...
        priority = volume_logic.priority

        if priority == VolumePriorityType.UNDEFINED:
            parent_id = volume_logic.parent_id or volume_logic.soft_parent_id
            priority = (
                VolumePriorityType.PRIMARY
                if self._is_primary_volume(parent_id) or self.active_primary_volume_id is None
    ...
    def remove_object_from_views(self, volume_logic: VolumeObjectLogic) -> None:
        ...
        if self._is_active_volume(volume_logic._id):
            ...
            if not is_next_volume_primary or next_volume_id is None:
